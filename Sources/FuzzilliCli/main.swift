...
let mainJSShellBinary = selectJSShellBinaryPath()
logger.info("Using the following binary for the target engine: \(mainJSShellBinary)")

func makeFuzzer(with configuration: Configuration, executablePath: String) -> Fuzzer {
    // A script runner to execute JavaScript code in an instrumented JS engine.
    let runner = REPRL(executable: executablePath, processArguments: jsShellArguments, processEnvironment: profile.processEnv, maxExecsBeforeRespawn: profile.maxExecsBeforeRespawn)
    ...
    return Fuzzer(configuration: configuration,
                  ...
                  corpus: corpus,
                  minimizer: minimizer,
                  crashProcessor: profile.optionalCrashProcessor)
}
...
let fuzzer = makeFuzzer(with: mainConfig, executablePath: mainJSShellBinary)
...
for jobId in 1..<numJobs {
    let workerBinary = selectJSShellBinaryPath()
    logger.info("Worker job \(jobId) will use the following binary for the target engine: \(workerBinary)")
    let worker = makeFuzzer(with: workerConfig, executablePath: workerBinary)
    worker.async {
