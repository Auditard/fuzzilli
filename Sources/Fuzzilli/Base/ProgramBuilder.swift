...
public class ProgramBuilder {
    ...
    private func generateTypeInternal(_ type: ILType) -> Variable {
        ...
        let typeGenerators: [(ILType, () -> Variable)] = [
            ...
            (.unboundFunction(), {
                    if let receiverType = type.receiver {
                        let receiverVar = self.generateTypeInternal(receiverType)
                        let methodNames = Array(receiverType.methods)
                        if !methodNames.isEmpty {
                            let matchingMethods = methodNames.filter {
                                self.fuzzer.environment.type(ofProperty: $0, on: receiverType).Is(type)
                            }
                            if let methodName = (matchingMethods.randomElement() ?? methodNames.randomElement()) {
                                let methodVar = self.getProperty(methodName, of: receiverVar)
                                if self.type(of: methodVar).Is(type) {
                                    return methodVar
                                }
                            }
                        }
                    }
                    return self.randomVariable(forUseAs: .unboundFunction())
                }),
            ...
            (.iterable, {
                    if let iterableVariable = self.randomVariable(ofTypeOrSubtype: .iterable) {
                        return iterableVariable
                    }
                    let elementCount = Int.random(in: 0...4)
                    if elementCount == 0 {
                        return self.createArray(with: [])
                    }
                    ...
                    for _ in 0..<elementCount {
                        ...
                        case 0:
                            elements.append(self.loadInt(self.randomInt()))
                        case 1:
                            elements.append(self.loadFloat(self.randomFloat()))
                        case 2:
                            elements.append(self.loadBool(probability(0.5)))
                        case 3:
                            elements.append(self.loadString(self.randomString()))
                        default:
                            elements.append(self.randomJsVariable())
                        }
                    }
                    return self.createArray(with: elements)
                }),
            (.wasmTypeDef(), {
            ...
            (.object(), {
                ...
                if generators.count > 0 {
                    ...
                    // wrong.
                    if let variable = self.randomVariable(ofTypeOrSubtype: type) {
                        return variable
                    }
                    // Fall back to any variable that can (probably) be used as the requested type instead of crashing.
                    return self.randomVariable(forUseAs: type)
                }
