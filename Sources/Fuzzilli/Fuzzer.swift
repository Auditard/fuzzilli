...
public class Fuzzer {
    ...
    /// Optional crash processor invoked before crashes are persisted.
    private let crashProcessor: FuzzingCrashProcessor?
    ...
    public init(
        ...
        environment: JavaScriptEnvironment, lifter: Lifter, corpus: Corpus, minimizer: Minimizer, queue: DispatchQueue? = nil, crashProcessor: FuzzingCrashProcessor? = nil
    ) {
        let uniqueId = UUID()
        self.id = uniqueId
        ...
        self.crashProcessor = crashProcessor
    ...
    func processCrash(_ program: Program, withSignal termsig: Int, withStderr stderr: String, withStdout stdout: String, origin: ProgramOrigin, withExectime exectime: TimeInterval) {
        func processCommon(_ program: Program) {
            ...
            if case .crashed = execution.outcome {
                ...
                // TODO: Get stat on ignored crahes
                let shouldDispatch = crashProcessor?.process(program,
                                                             withSignal: termsig,
                                                             withStderr: stderr,
                                                             withStdout: stdout,
                                                             origin: origin,
